...
public class JSGDActivity extends AppCompatActivity  implements View.OnClickListener, SGFingerPresentEvent {
	//private static final String TAG = "JSGDActivity";
    private static final String TAG = "JSGDActivity";
...
//    private android.widget.ToggleButton mToggleButtonCaptureModeN;
    ...
    private byte[] tomar_valor=new byte[0];
	private byte[] tomar_valor_dos=new byte[0];
...
//    private Switch mSwitchAutoOn;
				...
				synchronized (this) {
					...
					if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {
						if(device != null){
							...
							Intent launchActivityIntent = new Intent(context, JSGDActivity.class);
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		this.patientId = getIntent().getIntExtra("patientId", -1); // Modifica esta línea
		if (this.patientId != -1) {
			// Ahora patientId es un miembro de la clase y es accesible en otros métodos.
		}
	...
	//		mVerifyImage = null;
		...
		if (result == SGFDxErrorCode.SGFDX_ERROR_NONE) {
			...
			if (matched[0]) {
				...
				//Toast.makeText( JSGDActivity.this,"Verificado", Toast.LENGTH_SHORT).show();
				Toast.makeText( JSGDActivity.this,"Error al guardar la huella dactilar para el paciente con ID: " + patientId+" Huella: "+mRegisterTemplate, Toast.LENGTH_SHORT).show();
				new SaveFingerprintTask(patientId, mRegisterTemplate, JSGDActivity.this).execute();
				Intent intent = new Intent(JSGDActivity.this, GeneratePrescriptionActivity.class);
				startActivity(intent);
				finish();
			} else {
				mTextViewResult.setText("Fingerprint not matched!");
				Toast.makeText(JSGDActivity.this, "nada", Toast.LENGTH_SHORT).show();
	...
	private class SaveFingerprintTask extends AsyncTask<Void, Void, Boolean> {
		private int patientId;
		private byte[] fingerprintData;
		private Context context;
		public SaveFingerprintTask(int patientId, byte[] fingerprintData, Context context) {
		...
		protected Boolean doInBackground(Void... voids) {
			// Llamada a la función Kotlin desde Java

			return DatabaseConnection.saveFingerprintToDatabase(patientId, fingerprintData);
		}

		@Override
		protected void onPostExecute(Boolean result) {
			// Manejar el resultado aquí (e.g., mostrar un mensaje al usuario)
			if (!result) {
				//Toast.makeText(context, "Error al guardar la huella dactilar para el paciente con ID: " + patientId+" Huella: "+mRegisterTemplate, Toast.LENGTH_SHORT).show();
			}else {
				Toast.makeText(context, "Huella dactilar guardada exitosamente para el paciente con ID: " + patientId+" Huella: "+mRegisterTemplate, Toast.LENGTH_SHORT).show();
			}
		}
	}
}
